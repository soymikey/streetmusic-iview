  ...
  fetchHotEventList(override) {
    ...
    return getHotEventList(data)
      .then(res => {
        ...
        this.setState({
          hotEventList: override
            ? res.data.list
            : this.state.hotEventList.concat(res.data.list),
    ...
    return getHotSingerList(data)
      .then(res => {
        this.setState({
          hotList: override
            ? res.data.list
            : this.state.hotList.concat(res.data.list),
          total: res.data.total, //总页数
          loading: false,
        });
      })
      ...
      .then(res => {
        this.setState({
          recommendList: override
            ? res.data.list
            : this.state.recommendList.concat(res.data.list),
          total: res.data.total, //总页数
          loading: false,
        });
      })
          ...
          <View className='search-wrapper'>
            <View className='search' onClick={this.goToSearch.bind(this)}>
              ...
              <Image
                ...
                mode='aspectFit'
                className='search-left-icon'></Image>
            </View>
          </View>
          <Swiper
            style='height:250px;'
            ...
            autoplay>
            {hotEventList.length
              ? hotEventList.slice(0, 5).map(item => {
                  return (
                    <SwiperItem key={item.id}>
                      <View
                        className='image-wrapper'
                        onClick={this.goToEventDetailPage.bind(this, item.id)}>
                        <Image
                          src={item.poster[0]}
                          mode='aspectFit'
                          style='width: 100%'
